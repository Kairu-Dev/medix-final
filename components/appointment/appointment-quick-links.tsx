...
const AppointmentQuickLinks = async({ staffId, patientId }: { staffId: string, patientId: string }) => {
    ...
    console.log("AppointmentQuickLinks received staffId:", staffId);

    // If you're a doctor viewing a patient's page, ensure patientId is available
    let currentPatientId = patientId;
    
    // If doctor is viewing and no patientId provided, you might need to get it from URL or context
    // This is an example - adjust based on your actual data flow
    if (isDoctor && !currentPatientId) {
        // You could extract from URL params, context, or other sources
        // For example, if your page URL includes the patient ID
        const urlParams = new URLSearchParams(window.location.search);
        currentPatientId = urlParams.get('patientId') || '';
    }

    // For debugging
    console.log("Current patient ID for referral:", currentPatientId);
        ...
        <Card className="w-full rounded-xl bg-gray-900/60 border border-emerald-500/40 shadow-lg relative backdrop-blur-sm">  
            ...
            <CardContent className="flex flex-wrap gap-3 relative z-10">
                ...
                {isDoctor && currentPatientId && (
                    <ReferralForm doctorId={staffId} patientId={currentPatientId} />
                )}
            </CardContent>
        </Card>
    )
